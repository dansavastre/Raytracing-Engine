...
static glm::vec3 getFinalColor(const Scene& scene, const BoundingVolumeHierarchy& bvh, Ray ray)
{
    ...
    if (bvh.intersect(ray, hitInfo)) {
        ray.t = ray.t - 0.001f;
        color = calculateColor(scene, ray, hitInfo, bvh);
        glm::vec3 reflectionDirection = glm::reflect(ray.direction, hitInfo.normal);
        glm::vec3 reflectionRayOrigin = ray.origin + ray.t * ray.direction;
        Ray reflectionRay { reflectionRayOrigin, reflectionDirection };

        // normal debug ray
        Ray normalRay{ reflectionRayOrigin, hitInfo.normal };
        
        // drawRay(normalRay, { 0.0f,0.0f,1.0f });

        

        // check it the material is reflective and prevent infinite recursion
        if(hitInfo.material.ks != glm::vec3(0.0f) && ray.t > 0.00005){
            color = getFinalColor(scene, bvh, reflectionRay);
        }
        drawRay(ray, color);
    }
...
static void bilinearInterpolation(int x, int y, const Scene& scene, const BoundingVolumeHierarchy& bvh, Screen& screen, const Trackball& camera) 
{
    std::vector<glm::vec3> colors;
    for (int i = x; i < x + 2; i++) {
        for (int j = y; j < y + 2; j++) {
            glm::vec2 normalizedPixelPos{
                float(i) / windowResolution.x * 2.0f - 1.0f,
                float(j) / windowResolution.y * 2.0f - 1.0f
            };
    ...
    screen.setPixel(x, y, colors[0] * 0.25f + colors[1] * 0.25f + colors[2] * 0.25f + colors[3] * 0.25f);
    screen.setPixel(x, y+1, colors[0] * 0.25f + colors[1] * 0.25f + colors[2] * 0.25f + colors[3] * 0.25f);
    screen.setPixel(x+1, y, colors[0] * 0.25f + colors[1] * 0.25f + colors[2] * 0.25f + colors[3] * 0.25f);
    screen.setPixel(x+1, y+1, colors[0] * 0.25f + colors[1] * 0.25f + colors[2] * 0.25f + colors[3] * 0.25f);

}

static void setOpenGLMatrices(const Trackball& camera);
static void drawLightsOpenGL

(const Scene& scene, const Trackball& camera, int selectedLight);
...
static void renderRayTracing(const Scene& scene, const Trackball& camera, const BoundingVolumeHierarchy& bvh, Screen& screen, bool bilinearInterpolationBool)
{
#ifndef NDEBUG
    // Single threaded in debug mode
    if (bilinearInterpolationBool) {
        for (int y = 0; y < windowResolution.y; y = y + 2) {
            for (int x = 0; x != windowResolution.x; x = x + 2) {
                // NOTE: (-1, -1) at the bottom left of the screen, (+1, +1) at the top right of the screen.
                bilinearInterpolation(x, y, scene, bvh, screen, camera);
            }
        }
    }else {
...
int main(int argc, char** argv)
{
    ...
    int bvhDebugLevel = 0;
    bool debugBVH { false };
    bool bilinearInterpolationOption{ false };
    ...
    while (!window.shouldClose()) {
        ...
        if (ImGui::Button("Render to file")) {
            ...
            if (result == NFD_OKAY) {
                ...
                renderRayTracing(scene, camera, bvh, screen, bilinearInterpolationOption);
        ...
        ImGui::Text("Options");
        if (viewMode == ViewMode::Rasterization) {
            ImGui::Checkbox("Bilinear Interpolation", &bilinearInterpolationOption);
